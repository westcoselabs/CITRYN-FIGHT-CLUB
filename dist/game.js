...
// ===========================================================================
import { STR } from "./strings.js?v=mp-lobby-1";
import { Net, packHeld, bitsToInput } from "./net.js?v=mp-lobby-1";
...
function reseed(s) { _seed = (s >>> 0) || 1; }   // online: both clients share one seed
function rng() {
...
const SHEETS = {
  ...
  b_run:   { file: "Brandon/brandon-run.png", ft: 8 },
  ...
  g_run:   { file: "Garet/garet-run.png", ft: 8 },
  ...
  s_run:   { file: "Steven/steven-run.png", ft: 8 },
  ...
  t_run:   { file: "Tom/tom-run.png", ft: 8 },
...
// Measured from the MP4 movie headers and rounded up to 60 Hz simulation
// frames. Online clients must use these shared values instead of their local
// video metadata or the lockstep simulation can advance at different times.
const CUTSCENE_DURATION_FRAMES = {
  brandon: 601, // 10.006 s
  garet: 542,   //  9.033 s
  mo: 601,      // 10.006 s
  steven: 425,  //  7.082 s
  tom: 601,     // 10.006 s
...
function cutsceneDurationFrames(key) {
  if (CUTSCENE_DURATION_FRAMES[key]) return CUTSCENE_DURATION_FRAMES[key];
  const v = ensureCutscene(key);
...
// events originating from the lobby HTML overlay (typing a code, tapping a
// button) must NOT be intercepted by the game's keyboard/touch handling
const fromNetUI = (e) => !!(e && e.target && e.target.closest && e.target.closest("#netui"));
...
addEventListener("keyup", e => { if (fromNetUI(e)) return; const tok = CODE2TOK[e.code]; if (tok) heldKT.delete(tok); });
...
function beginRound() {
  ...
  match.specialFinishPoseSide = null;
  // online: both clients reseed identically per round so cosmetics also match
  if (match.online) reseed((match.netSeed ^ (match.roundNum * 0x9e3779b1)) >>> 0);
  ...
  if (match.online) {
    // fixed deterministic intro timing; the round voice plays cosmetically
    match.fightCueFrame = 84; match.fightStartFrame = 110;
  } else {
...
function updateFight() {
  ...
  if (match.phase === "fight") {
    ...
    let IN_A, IN_B;
    if (netInputs) { IN_A = netInputs.p1; IN_B = netInputs.p2; }   // online: relayed bitstream
    else { IN_A = a.isCPU ? aiInput(a, b) : humanInput(0); IN_B = b.isCPU ? aiInput(b, a) : humanInput(1); }
...
function render() {
  ...
  if (match.scene === "fight" || match.scene === "matchover") {
    ...
    drawHUD();
    drawBanner();
    if (match.scene === "fight" && !match.online) hintBar(controlsHintText());
    if (match.scene === "matchover") drawMatchOver();
    drawCinematicSpecialPresentation();
    if (match.online && match.scene === "fight") drawOnlineOverlay();
  } else {
    drawMenuScene();
  }
...
function drawMenuScene() {
  ...
  else if (match.scene === "online") drawOnlineEntry();
  else if (match.scene === "lobby") drawLobby();
}
...
function drawModeSelect() {
  neonText(STR.modeHeader, VW / 2, 86, 46, "#ffce4a", "#ff5c33");
  const items = [[STR.modeArcade, STR.modeArcadeDesc], [STR.modeVersus, STR.modeVersusDesc], [STR.modeTraining, STR.modeTrainingDesc], [STR.modeOnline, STR.modeOnlineDesc]];
  const w = 760, h = 86, x = VW / 2 - w / 2;
  items.forEach((it, i) => {
    const y = 168 + i * 108;
...
function updateMenus() {
  ...
  if (match.scene === "online") {
    // Create/Join are driven by the HTML overlay buttons; just handle Back here
    if (backPressed()) match.scene = "mode";
    return;
  }
  if (match.scene === "mode") {
    if (navEdge("up")) menu.modeIdx = (menu.modeIdx + MODES.length - 1) % MODES.length;
    if (navEdge("down")) menu.modeIdx = (menu.modeIdx + 1) % MODES.length;
    if (backPressed()) { match.scene = "title"; return; }
    if (confirmPressed()) {
      match.mode = MODES[menu.modeIdx];
      if (match.mode === "online") { openOnlineEntry(); return; }
...
function updateMatchOver() {
  ...
  if (confirmPressed()) {
    if (match.online) {
      if (menu.overIdx === 0) { Net.rematch(); onlineUI.locked = false; match.scene = "lobby"; clearTouch(); }
      else { leaveOnline(); }
      return;
    }
...
// ===========================================================================
//  ONLINE  (delay-based lockstep over the Durable Object relay)
// ===========================================================================
const NET_DELAY = 3;                 // fixed input-delay frames (smooths jitter)
let netInputs = null;                // {p1,p2} input objects for the current sim frame
const online = { simFrame: 0, inputFrame: 0, prevBits: [0, 0], stalled: 0, waiting: false, ended: false };
const onlineUI = { mySel: 0, locked: false, stageIdx: 0, status: "connecting", peerMsg: "", graceUntil: 0 };
let _netWired = false;

// one deterministic fight frame (hitstop/slowmo gate + the sim); shared by local
// and online so both run an identical sequence of operations per frame.
function simFightTick() {
...
// the local player always uses the P1 control scheme on their own machine
function localHeldBits() {
  return packHeld({
    left: held(0, "left"), right: held(0, "right"), up: held(0, "up"), down: held(0, "down"),
    punch: held(0, "punch"), kick: held(0, "kick"), block: held(0, "block"), special: held(0, "special"),
  });
}
function resetOnlineFight() {
  online.simFrame = 0; online.inputFrame = 0; online.prevBits = [0, 0];
  online.stalled = 0; online.waiting = false; online.ended = false; slowToggle = false;
  Net.clearInputs();
  for (let i = 0; i < NET_DELAY; i++) Net.submitInput(i, 0);   // pre-seed neutral frames = fixed delay
  online.inputFrame = NET_DELAY;
}
function onlineFightStep() {
  if (online.ended) return;
  Net.submitInput(online.inputFrame++, localHeldBits());        // queue local input for a future frame
  let advanced = 0;
  while (online.simFrame < online.inputFrame && advanced < 2) { // advance only on confirmed input pairs
    const pair = Net.inputsFor(online.simFrame);
    if (!pair) break;
    netInputs = { p1: bitsToInput(pair.p1, online.prevBits[0]), p2: bitsToInput(pair.p2, online.prevBits[1]) };
    online.prevBits[0] = pair.p1; online.prevBits[1] = pair.p2;
    simFightTick();
    netInputs = null;
    online.simFrame++; advanced++;
    if (online.simFrame % 30 === 0) Net.sendChecksum(online.simFrame, gameplayChecksum());
    Net.pruneInputs(online.simFrame - 4);
  }
  online.waiting = advanced === 0 && !online.ended;
  online.stalled = online.waiting ? online.stalled + 1 : 0;
}
const STATE_CODES = ["idle","walk","jump","crouch","block","attack","hit","knockdown","ko","win",
  "special","special_freeze","special_cinematic","special_hurt","special_win"];
function stateCode(s) { const i = STATE_CODES.indexOf(s); return i < 0 ? 98 : i; }
function gameplayChecksum() {
  const [a, b] = match.fighters; let h = 2166136261 >>> 0;
  const mix = (n) => { h ^= (n | 0); h = Math.imul(h, 16777619) >>> 0; };
  for (const f of [a, b]) {
    mix(f.x * 16 | 0); mix(f.y * 16 | 0); mix(f.hp); mix(f.meter); mix(f.facing);
    mix(f.stun); mix(f.invuln); mix(stateCode(f.state)); mix(f.atkF); mix(f.spF); mix(f.roundWins);
  }
  mix(match.timer); mix(match.timerF);
  mix(match.phase === "intro" ? 1 : match.phase === "fight" ? 2 : 3); mix(match.roundNum);
  const sq = match.specialSeq;
  mix(sq ? CINEMATIC_SPECIAL_PHASES.indexOf(sq.phase) + 1 : 0);
  if (sq) { mix(sq.phaseT); mix(sq.attackerSide); mix(sq.damageApplied ? 1 : 0); }
  return h >>> 0;
}

function wireNet() {
  if (_netWired) return; _netWired = true;
  Net.on("seat",   (m) => { match.mySeat = m.seat; match.netSeed = m.seed; onlineUI.status = m.seat < 0 ? "spectating" : m.status; });
  Net.on("lobby",  (m) => { onlineUI.status = m.status; });
  Net.on("conn",   (m) => { onlineUI.status = m.connected ? (Net.status === "running" ? "running" : "connected") : "reconnecting"; });
  Net.on("start",  (m) => {
    match.netSeed = m.seed; match.mySeat = Net.mySeat;
    match.p1char = m.p1char; match.p2char = m.p2char; match.stage = m.stage;
    onlineUI.peerMsg = ""; onlineUI.graceUntil = 0;
    startOnlineFight(m.score);
  });
  Net.on("peer",   (m) => {
    if (m.event === "left") { onlineUI.peerMsg = STR.netOppLeft; onlineUI.graceUntil = performance.now() + (m.graceMs || 12000); online.waiting = true; }
  });
  Net.on("forfeit",(m) => endOnlineByForfeit(m.winner));
  Net.on("desync", (m) => { console.warn("netcode desync @ frame", m.frame); });
  Net.on("error",  (m) => { console.warn("net error:", m.msg); });
}
function enterOnline(joinCode) {
  match.online = true; match.mode = "online";
  onlineUI.mySel = 0; onlineUI.locked = false; onlineUI.stageIdx = 0;
  onlineUI.status = "connecting"; onlineUI.peerMsg = ""; onlineUI.graceUntil = 0;
  wireNet();
  if (joinCode) Net.setRoom(joinCode); else Net.ensureRoom();
  Net.connect();
  match.scene = "lobby"; clearTouch();
}
function leaveOnline() {
  Net.disconnect(); match.online = false; match.mode = "arcade";
  match.scene = "mode"; clearTouch();
}

// ---------- online entry (Create / Join) HTML overlay wiring ----------
function openOnlineEntry() {
  match.online = false;
  match.scene = "online";
  const inp = document.getElementById("net-join-input");
  if (inp) inp.value = "";
  clearTouch();
}
let _netUIWired = false;
function setupNetUI() {
  if (_netUIWired) return; _netUIWired = true;
  const $ = (id) => document.getElementById(id);
  const txt = (id, t) => { const e = $(id); if (e && t) e.textContent = t; };
  txt("net-entry-h", STR.netOnlineEntryHeader); txt("net-create", STR.netCreate);
  txt("net-join", STR.netJoin); txt("net-copy", STR.netCopy); txt("net-share-btn", STR.netShare);
  txt("net-copied", STR.netCopied); txt("net-room-lbl", STR.netRoom);
  const inp = $("net-join-input"); if (inp && STR.netJoinPlaceholder) inp.placeholder = STR.netJoinPlaceholder;
  const doJoin = () => { const c = Net.parseRoomInput(inp ? inp.value : ""); if (c) enterOnline(c); };
  if ($("net-create")) $("net-create").onclick = () => enterOnline();
  if ($("net-join")) $("net-join").onclick = doJoin;
  if (inp) inp.addEventListener("keydown", (e) => { if (e.key === "Enter") doJoin(); });
  if ($("net-entry-back")) $("net-entry-back").onclick = () => { match.scene = "mode"; };
  if ($("net-copy")) $("net-copy").onclick = doCopyInvite;
  if ($("net-share-btn")) $("net-share-btn").onclick = () => {
    if (Net.canShare()) navigator.share({ title: "Citryn Fight Club", url: Net.inviteLink() }).catch(() => {});
  };
}
let _lastNetScene = null;
function syncNetUI() {
  if (match.scene === _lastNetScene) return;
  _lastNetScene = match.scene;
  const ui = document.getElementById("netui"), entry = document.getElementById("net-entry"), share = document.getElementById("net-share");
  if (!ui || !entry || !share) return;
  const showEntry = match.scene === "online", showShare = match.scene === "lobby";
  ui.style.display = (showEntry || showShare) ? "block" : "none";
  entry.style.display = showEntry ? "block" : "none";
  share.style.display = showShare ? "flex" : "none";
  if (showShare) {
    const code = document.getElementById("net-room-code"); if (code) code.textContent = Net.room || "—";
    const sb = document.getElementById("net-share-btn"); if (sb) sb.style.display = Net.canShare() ? "" : "none";
  }
}
function doCopyInvite() {
  const link = Net.inviteLink();
  const toast = () => { const t = document.getElementById("net-copied"); if (t) { t.classList.add("show"); setTimeout(() => t.classList.remove("show"), 1400); } };
  if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(link).then(toast).catch(() => fallbackCopy(link, toast));
  else fallbackCopy(link, toast);
}
function fallbackCopy(text, done) {
  try {
    ...
    document.execCommand("copy"); document.body.removeChild(ta); if (done) done();
  } catch (e) {}
}
function drawOnlineEntry() {
  neonText(STR.modeOnline, VW / 2, 90, 44, "#ffce4a", "#ff5c33");
}
function startOnlineFight(score) {
  match.fighters[0] = makeFighter(match.p1char, 0, false);
  match.fighters[1] = makeFighter(match.p2char, 1, false);
  warmCutscene(match.fighters[0].cd.cinematic.cutscene);
  warmCutscene(match.fighters[1].cd.cinematic.cutscene);
  match.fighters[0].roundWins = score ? (score[0] | 0) : 0;
  match.fighters[1].roundWins = score ? (score[1] | 0) : 0;
  match.roundNum = match.fighters[0].roundWins + match.fighters[1].roundWins + 1;
  match.specialFinishPoseSide = null;
  match.scene = "fight";
  resetOnlineFight();
  beginRound();
}
function endOnlineByForfeit(winner) {
  online.ended = true; match.winner = winner; menu.overIdx = 0;
  if (match.fighters[winner]) match.fighters[winner].state = "win";
  match.scene = "matchover";
}

// ---------- lobby (networked character + stage pick) ----------
function updateLobby() {
  if (backPressed()) { leaveOnline(); return; }
  if (!onlineUI.locked) {
    if (navEdge("left")) onlineUI.mySel = (onlineUI.mySel + CHAR_LIST.length - 1) % CHAR_LIST.length;
    if (navEdge("right")) onlineUI.mySel = (onlineUI.mySel + 1) % CHAR_LIST.length;
  }
  // host (seat 0) picks the stage
  if (match.mySeat === 0) {
    if (navEdge("up")) { onlineUI.stageIdx = (onlineUI.stageIdx + STAGES.length - 1) % STAGES.length; Net.setStage(onlineUI.stageIdx); }
    if (navEdge("down")) { onlineUI.stageIdx = (onlineUI.stageIdx + 1) % STAGES.length; Net.setStage(onlineUI.stageIdx); }
  }
  if (confirmPressed()) {
    if (match.mySeat < 0) return;                 // spectators cannot ready
    if (!onlineUI.locked) { Net.select(CHAR_LIST[onlineUI.mySel]); if (match.mySeat === 0) Net.setStage(onlineUI.stageIdx); Net.ready(true); onlineUI.locked = true; }
    else { Net.ready(false); onlineUI.locked = false; }
  }
}

function drawLobby() {
  // the room code + Copy/Share buttons live in the #net-share HTML strip at the top
  // of the screen, because canvas text can't be selected or copied
  neonText(STR.netLobbyHeader, VW / 2, 98, 34, "#ffce4a", "#ff5c33");
  ctx.save(); ctx.font = "700 15px 'Trebuchet MS'"; ctx.fillStyle = "#9bffec"; ctx.textAlign = "center";
  ctx.fillText(statusLabel(), VW / 2, 132); ctx.restore();

  // seat summary
  const lp = (Net.lobby && Net.lobby.players) || [{ seat: 0 }, { seat: 1 }];
  [0, 1].forEach((s) => {
    const p = lp[s] || {}; const x = s === 0 ? VW * 0.27 : VW * 0.73;
    const col = s === 0 ? "#ff7a33" : "#00e5ff";
    const label = (s === match.mySeat ? STR.netYou + " " : "") + "P" + (s + 1);
    neonText(label, x, 168, 22, col, col);
    const txt = p.char ? CHARS[p.char].name : (p.connected ? STR.netChoosing : STR.netWaiting);
    neonText(txt + (p.ready ? "  ✔" : ""), x, 196, 24, p.ready ? "#7cff9b" : "#fff", col);
  });

  // character grid (local player's pick)
  const n = CHAR_LIST.length;
  const w = 150, h = 230, gap = 16;
  const totalW = w * n + gap * (n - 1), x0 = VW / 2 - totalW / 2, y = 236;
  CHAR_LIST.forEach((ck, i) => {
    const x = x0 + i * (w + gap);
    const sel = onlineUI.mySel === i;
    drawCharPortrait(ck, x, y, w, h, sel, sel ? (match.mySeat === 1 ? "#00e5ff" : "#ff7a33") : "#3a2f55", "", sel && onlineUI.locked);
  });

  // stage line
  const stName = STAGES[match.mySeat === 0 ? onlineUI.stageIdx : (Net.stage || 0)].name;
  neonText((match.mySeat === 0 ? STR.netStageHost : STR.netStageGuest) + " " + stName, VW / 2, y + h + 34, 22, "#9bffec", "#00e5ff");

  // prompt
  const prompt = match.mySeat < 0 ? STR.netSpectator
    : onlineUI.locked ? STR.netWaitingReady : STR.netPick;
  if ((frameCount >> 4) % 2 || onlineUI.locked) neonText(prompt, VW / 2, VH - 64, 24, "#fff", "#ff7a33");
  hintBar(STR.netLobbyHint);
}
function statusLabel() {
  if (onlineUI.status === "reconnecting") return STR.netReconnecting;
  if (onlineUI.status === "connecting") return STR.netConnecting;
  if (Net.latency) return STR.netPing + " " + Net.latency + "ms";
  return STR.netConnected;
}

function drawOnlineOverlay() {
  // ping
  ctx.save(); ctx.font = "700 14px 'Trebuchet MS'"; ctx.textAlign = "right"; ctx.fillStyle = "#9bffec";
  ctx.fillText((Net.latency ? Net.latency + "ms" : "--") + "  " + (match.mySeat === 0 ? "P1 (YOU)" : "P2 (YOU)"), VW - 14, VH - 12);
  ctx.restore();
  // waiting / reconnect overlay
  if (online.waiting && online.stalled > 8) {
    ctx.save(); ctx.fillStyle = "rgba(8,6,16,0.6)"; ctx.fillRect(0, 0, VW, VH);
    let msg = STR.netWaitingOpp;
    if (onlineUI.graceUntil) {
      const left = Math.max(0, Math.ceil((onlineUI.graceUntil - performance.now()) / 1000));
      msg = (onlineUI.peerMsg || STR.netOppLeft) + " (" + left + ")";
    }
    neonText(msg, VW / 2, VH / 2, 40, "#ffce4a", "#ff5c33");
    ctx.restore();
  }
}
...
function step() {
  ...
  // pause / quit-to-menu only in LOCAL fights (you can't pause one side online)
  if (match.scene === "fight" && !match.online) {
  ...
  if (match.scene === "fight") {
    if (match.online) onlineFightStep();          // lockstep: advances only on confirmed inputs
    else if (!paused) simFightTick();
  } else if (match.scene === "matchover") {
    updateMatchOver();
    if (fx.shakeT > 0) { fx.shakeT--; fx.shakeMag *= 0.86; }
  } else if (match.scene === "lobby") {
    updateLobby();
  } else {
    updateMenus();
  }
...
async function boot() {
  ...
  setupNetUI();                 // wire the lobby overlay buttons once
  ...
  // opened via an invite link (?room=...) -> drop straight into the online lobby
  if (Net.roomFromUrl()) enterOnline();
  requestAnimationFrame(frame);
}
boot();
